...
of the display — shelf endcap, bulk crate, or case stack.

![Shelf display mockup](data/pudding_endcap_2025_mockup.png)
...
and a feedback loop that learns from past runs.
...
- **Learning** — log what actually happened after a display runs, and
  ...
  fitted elasticity — plain averages and a small regression, nothing
  fancy).
- **Storage** — SQLite (`data/planner.db`). CSVs are still how you edit
  input data; they get synced into the DB on each run.
...
| Shelf endcap | Bulk crate | Case stack |
|---|---|---|
| ![shelf](data/pudding_endcap_2025_mockup.png) | ![crate](data/produce_crate_2025_mockup.png) | ![stack](data/soda_stack_2025_mockup.png) |

Same pipeline, three different physical fixtures — the shelf display
uses facing placement with eye-level priority, the crate display is a
bulk volume calculation, the case stack is whole-case footprint math.

## Tech stack

Python 3.10+, SQLite (stdlib), [PuLP](https://coin-or.github.io/pulp/)
for the linear program behind the shelf optimizer, matplotlib for the
mockups, pytest for the test suite (167 tests).
...
case_height_in`. Only the first few are required, the rest have
sensible defaults. Case dimensions only matter for a side-stack
display.
...
- The eye-level shelf weighting (`shelf_visibility_weight` in
  ...
  Same with the diminishing-returns curve the LP optimizer uses — total
  guess at the shape. Both would be worth fitting from real numbers
  once there's enough logged history.
...
- Real ML forecasting isn't worth it yet — not enough logged displays
  ...
  that's there now (plain averages + a small regression) is doing fine
